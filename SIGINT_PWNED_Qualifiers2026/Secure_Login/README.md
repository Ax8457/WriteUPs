...
<p align="justify">The Weakness in the script used to generate RSA keys lies on the fact that $p$ and $q$ are very closed because $q$ is actually the first prime found after $p$. The <a href="https://en.wikipedia.org/wiki/Fermat's_factorization_method">Fermat's Factorization</a> states that if $p$ and $q$ are closed enough, it's easy to factorize $N$. $N$ can be written as an odd integer: </p>
